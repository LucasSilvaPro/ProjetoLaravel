				...
				<table class='uk-table uk-table-middle uk-table-divider' border='2' width='80%'>
					...
					<tr>
						...
						@foreach($alunos as $aluno)
								<center>
								<tr>									
							 		<td><center>{{$aluno->getNome_Aluno()}}</center></td>
					
							 	
							 		<td>
							 			@if($aluno->getCurso_Id()==0)

							 				<center><a href="">Matricular</a></center>
							 			@endif
							 		</td>							 									 		
							 		<td><center>{{$aluno->getEmail_Aluno()}}</center></td>
								 	<td><center>
								 			<form action="{{route('atualizarAluno' ,$aluno->getId())}}" >							
												<div class='uk-margin'>
									       			<div class='uk-inline'>
									            		<span class='uk-form-icon' uk-icon='icon: user'></span>
														<input class='uk-input' type='text' name='nome_aluno' value='{{$aluno->getNome_Aluno()}}' placeholder='nome do Aluno' required/>
									        		</div>
									    		</div>
									    		<div class='uk-margin'>
									       			<div class='uk-inline'>
														...
														<input class='uk-input' type='text' name='email_aluno' value='{{$aluno->getEmail_Aluno()}}' placeholder='status do Curso' required/>
									        		</div>
									    		</div>
								 		...
								 		<td><center>											
									            <a href="{{route('deletar' ,$aluno->getId())}}"><button class='uk-button uk-button-default'>Apagar</button></a>
